...
allIntegersExcludingFirst a b = if b >= a then drop 1 [a..b] else drop 1 [a,a-1..b]
...
numbertostring a = [x:[] | x <- (show a)]

stringtointegers a = map (read :: String -> Int) [x:[] | x <- (show a)]

isRigid n = sum (stringtointegers n) == product (stringtointegers n)
